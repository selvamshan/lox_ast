...
impl<'a> Parser<'a> {
    ...
    fn statement(&mut self) -> Result<Rc<Stmt>, LoxResult> {
        ...
        if self.is_match(&[TokenType::Return]) {
            return Ok(Rc::new(self.return_statment()?));
        }
    ...
    fn for_statement(&mut self) -> Result<Rc<Stmt>, LoxResult> {
         ...
         let initiallizer = if self.is_match(&[TokenType::SemiColon]) {
             None
         }  else if self.is_match(&[TokenType::Var]) {           
         ...
         let increment  = if self.is_match(&[TokenType::RightParen])  {
            None
         } else {            
            Some(self.expression()?)
         };
    ...
    fn if_statement(&mut self) -> Result<Stmt, LoxResult> {
        ...
        Ok(Stmt::If(Rc::new(IfStmt{condition:Rc::new(condition), then_branch, else_branch})))       
    ...
    fn print_statement(&mut self) -> Result<Stmt, LoxResult> {
        ...
        Ok(Stmt::Print(Rc::new(PrintStmt { expression: Rc::new(value) })))
    }
    ...
    fn var_declaration(&mut self) -> Result<Rc<Stmt>, LoxResult> {
        ...
        Ok(Rc::new(Stmt::Var(Rc::new(VarStmt {
            name: name.clone(),
            initializer,
        }))))
    }
    ...
    fn function(&mut self, kind:&str) -> Result<Rc<Stmt>, LoxResult>{
        ...
        Ok(Rc::new(Stmt::Function(Rc::new(FunctionStmt
             { name, params:Rc::new(params), body }))))
    ...
    fn or(&mut self) -> Result<Expr, LoxResult> {
        ...
        while self.is_match(&[TokenType::Or]) {
            ...
            expr = Expr::Logical(Rc::new(LogicalExpr{left: Rc::new(expr), operator, right}));
        }
        Ok(expr)
    }
    ...
    fn finish_call(&mut self, callee: &Rc<Expr>)  -> Result<Expr, LoxResult> {
        ...
        let paren = self.consume(TokenType::RightParen, "Exprec ')' after arguments")?;

        Ok(Expr::Call(Rc::new(CallExpr { callee: Rc::clone(callee), paren, arguments })))
    }
